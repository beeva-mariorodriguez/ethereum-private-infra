...
resource "aws_api_gateway_integration" "enode" {
  ...
  type                    = "HTTP_PROXY"
  uri                     = "http://${aws_instance.ethereum_proxy.public_ip}:8080"
}
...
resource "aws_api_gateway_integration" "json" {
  rest_api_id = "${aws_api_gateway_rest_api.ethereum.id}"
  resource_id = "${aws_api_gateway_method.json.resource_id}"
  http_method = "${aws_api_gateway_method.json.http_method}"

  integration_http_method = "ANY"
  type                    = "HTTP_PROXY"
  uri                     = "http://${aws_instance.bastion.public_ip}:8080/InnovationDay.json"
}
